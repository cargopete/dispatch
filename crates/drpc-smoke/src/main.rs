//! drpc-smoke — end-to-end smoke test for a live dRPC provider.
//!
...
//!
//! Usage:
//!   cargo run --bin drpc-smoke -- --endpoint https://rpc.cargopete.com --chain-id 42161
//!
//! Environment variables (all optional — defaults target the live network):
//!   DRPC_ENDPOINT          Provider base URL              (default: https://rpc.cargopete.com)
//!   DRPC_CHAIN_ID          EIP-155 chain ID               (default: 42161)
//!   DRPC_DATA_SERVICE      RPCDataService address         (default: live mainnet address)
//!   DRPC_TALLY_COLLECTOR   GraphTallyCollector address    (default: live mainnet address)
//!   DRPC_SIGNER_KEY        Hex private key for receipts   (default: random ephemeral key)
...
async fn main() -> Result<()> {
    let endpoint = std::env::var("DRPC_ENDPOINT").unwrap_or_else(|_| DEFAULT_ENDPOINT.to_string());
    let chain_id: u64 = std::env::var("DRPC_CHAIN_ID")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(DEFAULT_CHAIN_ID);

    let data_service: Address = std::env::var("DRPC_DATA_SERVICE")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(DATA_SERVICE);

    let tally_collector: Address = std::env::var("DRPC_TALLY_COLLECTOR")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(TALLY_COLLECTOR);

    // Use provided key or generate a fresh ephemeral one.
    let signing_key = match std::env::var("DRPC_SIGNER_KEY") {
        Ok(hex) => {
            let bytes = hex::decode(hex.trim_start_matches("0x"))
                .context("DRPC_SIGNER_KEY: invalid hex")?;
            SigningKey::from_slice(&bytes).context("DRPC_SIGNER_KEY: invalid key")?
        }
        Err(_) => SigningKey::random(&mut OsRng),
    };

    let signer_address = drpc_tap::address_from_key(&signing_key);
    let domain_sep = drpc_tap::domain_separator("TAP", EIP712_CHAIN_ID, tally_collector);
    ...
    for (i, test) in tests().iter().enumerate() {
        ...
        let receipt = drpc_tap::create_receipt(
            &signing_key,
            domain_sep,
            data_service,
            ...
            // To do a full validated test, set DRPC_PROVIDER_ADDRESS.
            provider_address(),
            receipt_value,
            Bytes::default(),
        )?;
...
fn provider_address() -> Address {
    std::env::var("DRPC_PROVIDER_ADDRESS")
        .ok()
        .and_then(|s| s.parse().ok())
